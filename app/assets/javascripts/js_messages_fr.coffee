define [], ->
	LangFR = {
  		"huntArea":"Zone de la chasse",
		"stepStart":"Départ de l'étape",
		"end":"Arrivée",
		"goodAnswer":"Vous venez de trouvez la bonne réponse, vous pouvez continuer !",
		"goodPlace":"Vous êtes au bon endroit, vous pouvez continuer !",
		"badAnswer":"Ce n'est pas la bonne réponse, mais persévérez, vous trouverez !",
		"badPlace":"Vous n\'êtes pas au bon endroit, continuez de chercher !",
		"passEnigma":"Voulez-vous vraiment passer cette énigme ?",
		"stop":"Arrêt : ",
		"start":"Départ",
		"monuments":"Monuments",
		"trees":"Arbres",
		"greenSpaces":"Espaces verts",
		"fountains":"Fontaines",
		"gardens":"Jardins",
		"publicPlaces":"Lieux publics",
		"wikiLink":"Lien vers la page Wikipédia",
		"lawns":"pelouse(s)",
		"flowers":"fleur(s)",
		"shrubs":"arbuste(s)",
		"games":"jeu(x)",
		"plants":"Végétaux",
		"fountain":"Fontaine ",
		"districtName":"Nom du quartier : ",
		"sure":"Êtes-vous sûr de vous ?",
		"en":"Anglais",
		"fr":"Français"
	}
	
	return LangFR